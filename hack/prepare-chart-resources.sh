...
function get_crd_file_names() {
  if is_k8s_version_ge_1_29; then
   declare -a crds=("druid.gardener.cloud_etcds.yaml" "druid.gardener.cloud_etcdcopybackupstasks.yaml" "druid.gardener.cloud_etcdoperatortasks.yaml")
  else
   declare -a crds=("druid.gardener.cloud_etcds_without_cel.yaml" "druid.gardener.cloud_etcdcopybackupstasks.yaml" "druid.gardener.cloud_etcdoperatortasks.yaml")
  fi
  echo "${crds[*]}"
}
