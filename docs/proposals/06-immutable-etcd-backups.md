...
> [!NOTE]
> In AWS S3, changes to the bucket-level immutability period can be blocked by a specific bucket policy.

### Recommended Approach

A comprehensive assessment of immutability features across major cloud storage providers reveals several constraints for object-level immutability:

- **S3 and ABS:** Both typically require configuring bucket-level immutability before object-level immutability can be enabled.
- **GCS:** Does not currently support object-level immutability for existing buckets (see [issue](https://issuetracker.google.com/issues/346679415?pli=1)).
- **ABS:** Requires a migration process for existing containers before version-level immutability can be enabled, as outlined [here](https://learn.microsoft.com/en-us/azure/storage/blobs/immutable-version-level-worm-policies#migration).
...
1. **Prerequisite: Capture a Final Full Snapshot Prior to Hibernation**  
...
2. **Utilize an Operator Task to Re-Upload the latest full snapshot**  
...
A new field can be added in `etcd.spec.backup.store` to indicate the chosen immutability strategy:
...
type ImmutabilitySpec struct {
    // RetentionType indicates the type of immutability approach (e.g. "Bucket").
    RetentionType string `json:"retentionType,omitempty"`
}
```

By default, if `immutability` is not specified, no immutability behavior is enforced.
...
type ExtendFullSnapshotImmutabilityTaskConfig struct {
  // Schedule defines a cron schedule for full snapshots (e.g., "0 */6 * * *").
  schedule *string `json:"schedule,omitempty"`

  // GarbageCollectionConfig specifies the configuration for snapshot garbage collection.
  GarbageCollectionConfig *GarbageCollectionConfig `json:"garbageCollectionConfig,omitempty"`
}

type GarbageCollectionConfig struct {
  // GarbageCollectionPolicy indicates which policy to use (e.g., "Exponential" or "LimitBased").
  GarbageCollectionPolicy *string `json:"garbageCollectionPolicy,omitempty"`
  // MaxBackupsLimitBasedGC sets the maximum number of full snapshots to keep. Any snapshots exceeding this limit are deleted.
  MaxBackupsLimitBasedGC *int32 `json:"maxBackupsLimitBasedGC,omitempty"`
  // DeltaSnapshotRetentionPeriod is the duration for which delta snapshots are retained, excluding the latest snapshot set.
  // This must be a string representing a duration (e.g., "1s", "2m", "3h", "4d").
  DeltaSnapshotRetentionPeriod *time.Duration `json:"deltaSnapshotRetentionPeriod,omitempty"`
}
```
