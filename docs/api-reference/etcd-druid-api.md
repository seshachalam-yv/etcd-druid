...
| `metadata` _[ObjectMeta](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.29/#objectmeta-v1-meta)_ | Refer to Kubernetes API documentation for fields of `metadata`. |  |  |
| `spec` _[EtcdOpsTaskSpec](#etcdopstaskspec)_ | Spec defines the desired state of the EtcdOpsTask resource. |  | Required: \{\} <br /> |
| `status` _[EtcdOpsTaskStatus](#etcdopstaskstatus)_ | Status is the most recently observed status of the EtcdOpsTask resource. |  |  |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `config` _[EtcdOpsTaskConfig](#etcdopstaskconfig)_ | Config defines the configuration for the EtcdOpsTask.<br />Only one of the configurations can be specified at a time. |  | Required: \{\} <br /> |
...
| `lastErrors` _[EtcdOpsTaskLastError](#etcdopstasklasterror) array_ | LastErrors represents the errors when processing the task. |  | MaxItems: 10 <br /> |
| `lastOperation` _[EtcdOpsLastOperation](#etcdopslastoperation)_ | LastOperation captures the last operation status if task involves many stages. |  |  |
