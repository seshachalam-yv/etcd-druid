	...
	Eventually(func() error {
		...
		if *etcd.Status.ClusterSize != etcd.Spec.Replicas {
			return fmt.Errorf("etcd %s cluster size is %v, but its not expected size as %v",
				etcd.Name, etcd.Status.ClusterSize, multiNodeEtcdReplicas)
		}
...
func deleteAndCheckEtcd(ctx context.Context, cl client.Client, logger logr.Logger, etcd *v1alpha1.Etcd) {
	...
	// r, _ := k8s_labels.NewRequirement("instance", selection.Equals, []string{etcd.Name})
	// opts := &client.ListOptions{LabelSelector: k8s_labels.NewSelector().Add(*r)}

	logger.Info("Checking if statefulset pvc is gone")
	...
	// 	logger.Info(fmt.Sprintf("PVC::%v", len(pvcList.Items)))
	// 	if len(pvcList.Items) != 0 {
	// 		return fmt.Errorf("PVC is being deleting")
	// 	}
	// 	for _, pvc := range pvcList.Items {
	// 		if err := kutil.DeleteObject(ctx, cl, &pvc); err != nil {
	// 			return err
	// 		}
	// 	}
	// 	return nil
	...
	// logger.Info("waiting smooth deletion")
